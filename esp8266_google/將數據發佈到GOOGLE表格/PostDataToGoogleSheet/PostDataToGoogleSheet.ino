...
//在此設定時間間隔推送數據
const int dataPostDelay = 15000;  // 15 minutes = 15 * 60 * 1000=900000
...
function save_data(tag, value){
  ...
  try {
    ...
    var summarySheet = ss.getSheetByName("Summary");//必須與工作表名稱匹配
    var dataLoggerSheet = ss.getSheetByName("DataLogger");//必須與工作表名稱匹配
